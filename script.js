// ML Concentration Game - Complete Implementation
class MLConcentrationGame {
    constructor() {
        this.cards = [];
        this.gameCards = [];
        this.flippedCards = [];
    ...
    loadSettings() {
        const saved = localStorage.getItem('mlGameSettings');
        if (saved) {
    ...
    saveSettings() {
        localStorage.setItem('mlGameSettings', JSON.stringify(this.settings));
    }
    ...
    useDefaultCards() {
        this.cards = [
            { id: "1", command: "Neural Network", syntax: "nn.Linear(in, out)", description: "A computing system inspired by biological neural networks", example: "model = nn.Sequential(nn.Linear(784, 128))", category: "Architecture", explanation: "Neural networks are computing systems inspired by biological neural networks that constitute animal brains." }
        ];
    }
    ...
    displayPreGameCards() {
        ...
        this.displayCards.forEach((card, index) => {
            ...
            pair.innerHTML = `
                <div class="playing-card concept-card ${this.isCommandHidden ? 'flipped' : ''}">
                    <div class="card-face card-front">
                        <div class="card-category">${card.category || 'ML'}</div>
    ...
    flipCommands() {
        ...
        document.querySelectorAll('.concept-card').forEach(c => c.classList.toggle('flipped', this.isCommandHidden));
        const btn = document.getElementById('flipCommandBtn');
        if (btn) btn.textContent = this.isCommandHidden ? '👀 Show Concepts' : '🙈 Hide Concepts';
    }
    ...
    resetFlips() {
        this.isCommandHidden = false;
        this.isExplanationHidden = false;
        document.querySelectorAll('.concept-card').forEach(c => c.classList.remove('flipped'));
        ...
        if (cmdBtn) cmdBtn.textContent = '🙈 Hide Concepts';
                ...
                cardEl.innerHTML = `
                    <div class="card-inner">
                        <div class="card-front">
                            <div class="card-category-tag">${card.category || 'ML'}</div>
                            <div class="card-command-text">${card.command}</div>
                        </div>
                        <div class="card-back"><span>ML</span></div>
                    </div>
                `;
            } else {
                cardEl.innerHTML = `
        ...
        cardsToUse.forEach((card, index) => {
            ...
            item.innerHTML = `
                <div class="listen-item-header">
                    ...
                    <div class="listen-category">${card.category || 'ML'}</div>
    ...
    async playAllListen() {
        ...
        for (let i = 0; i < cardsToUse.length && this.isPlayingAll; i++) {
            ...
            await this.playListenItem(card, i);
    ...
    async askAIAboutCommand(card) {
        ...
        content.innerHTML = '<div class="ai-loading">Analyzing ML concept...</div>';
        ...
        try {
            const response = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'user',
                        content: `Explain this Machine Learning concept in detail for a learner:
...
Please provide:
1. **What it is** - Clear explanation of the concept's purpose
2. **How it works** - Technical details and mechanisms
3. **When to use it** - Common scenarios
4. **Best practices** - Tips for using it effectively
5. **Related concepts** - Similar or complementary ML concepts
...
Format with clear headings and code examples.`
            ...
            content.innerHTML = '<div class="ai-error">Failed to get AI explanation. Check your API key.</div>';
        }
    }
    ...
    showPracticeCard() {
        ...
        if (catEl) catEl.textContent = card.category || 'ML';
    ...
    checkPracticeAnswer() {
        ...
        const correctAnswer = this.practiceCards[this.practiceIndex].command.toLowerCase();
    ...
    showLibrary() {
        ...
        this.library.decks.forEach(deck => {
            ...
            item.innerHTML = `
                ...
                <div class="library-item-count">${deck.cardCount} concepts • ${deck.difficulty || 'all levels'}</div>
            `;
        ...
        reader.onload = (e) => {
            try {
                ...
                if (data.cards && Array.isArray(data.cards)) {
                    ...
                    this.displayPreGameCards();
                    this.displayListenList();
                    alert(`Imported ${data.cards.length} ML concepts!`);
                } else {
    ...
    setupEventListeners() {
        ...
        document.getElementById('saveSettings')?.addEventListener('click', () => {
            this.settings.matchedPairBehavior = document.getElementById('matchedPairBehavior')?.value || 'stay';
            this.settings.autoAdvance = document.getElementById('autoAdvance')?.checked || false;
            this.settings.timerDuration = parseInt(document.getElementById('timerDuration')?.value) || 60;
            this.settings.openaiApiKey = document.getElementById('openaiApiKey')?.value || '';
            this.saveSettings();
            document.getElementById('settingsModal').style.display = 'none';
        });
...
// Initialize the game
const game = new MLConcentrationGame();
